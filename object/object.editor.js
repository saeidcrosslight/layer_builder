        ...
        .factory('editor', function (file) {
            var factory = {},
                    editorContainerObject = function () {
                        ...
                        this.removeCommentOut = removeCommentOut;
                    },
                    hideAllEditor = function (editorArrayObject) { //editorArrayObject -- $(".editor_csuprem") or $(".editor_apsys")
                    ...
                    createEdtior = function (product, fileName, filePath, editorID, editorContainerID, editorArrayObject) {
                        ...
                        if(file.extensionName(fileName) === ".layer"){
                            fileCurrentContent = fileCurrentContent.replace(/\n\s+/g,'\n\n');
                        }
                        ...
                        if (product.productName === "CSuprem") //3.
                            setMode_CSuprem(editorID);
                        else
                            setMode_Apsys(editorID);
                    ...
                    setMode_CSuprem = function (editorID) {
                        var editor = ace.edit(editorID);
                        editor.setTheme("ace/theme/csuprem");
                        editor.getSession().setMode("ace/mode/csuprem");
                    },
                    setMode_Apsys = function (editorID) {
                        var editor = ace.edit(editorID);
                    ...
                    hasTheFileInProject = function(product, fileType){
                        ...
                        if(product.projectPath !== "")
                            files = file.readfoldersync(product.projectPath);
                    ...
                    commentOut = function(product){
                        var currentEditor = ace.edit(product.editors.getCurrentEditorObject(product).editorID),
                            startLineNumber = currentEditor.selection.getRange().start.row+1,
                            endLineNumber = currentEditor.selection.getRange().end.row+1,
                            comment = "$";
                        if(product.productName === "CSuprem")
                            comment = "#";
                        if(startLineNumber !== endLineNumber){ //more line
                            for(var i=startLineNumber; i<endLineNumber+1; i++){
                                currentEditor.gotoLine(i);
                                currentEditor.insert(comment);
                            }
                        }else{ //only one line
                            currentEditor.gotoLine(startLineNumber);
                            currentEditor.insert(comment);
                        }                        
                    },
                    removeCommentOut = function(product){
                        var eid = product.editors.getCurrentEditorObject(product).editorID,
                            currentEditor = ace.edit(eid),
                            startLineNumber = currentEditor.selection.getRange().start.row+1,
                            endLineNumber = currentEditor.selection.getRange().end.row+1,
                            currentPageStartLine = $($("div #editorContainer_"+product.productName+" #"+eid+" .ace_gutter-cell ")[0]).text();
                        if(startLineNumber !== endLineNumber){ //more line
                            for(var i=startLineNumber; i<endLineNumber+1; i++){
                                var linecontent = $($("div #editorContainer_"+product.productName+" #"+eid+" .ace_line")[i-currentPageStartLine]).text();
                            ...
                            var linecontent = $($("div #editorContainer_"+product.productName+" #"+eid+" .ace_line")[startLineNumber-1]).text();
                            if(linecontent[0]==="#" || linecontent[0]==="$"){
                                currentEditor.gotoLine(startLineNumber);
                                currentEditor.remove();
                            }
